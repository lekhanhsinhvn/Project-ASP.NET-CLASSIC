...
const GETORDERS = gql`
  query GetOrders($pageNum: Int, $maxPerPage: Int, $search: String, $sort: String, $asc: Boolean){
    getOrders(pageNum: $pageNum, maxPerPage: $maxPerPage, search: $search, sort: $sort, asc: $asc) {
        orderId
        inferior{
          userId
          name
...
class InferiorOrderTable extends React.Component {
  ...
  getOrders({
    ...
    client.query({
      ...
      .then((response) => {
        dataOrders = response.data.getOrders;
        if (!_.isEqual(orders, dataOrders)) {
          client.query({
            query: GETTOTALCOUNTORDER,
            errorPolicy: 'ignore',
          }).then((response1 = response) => {
            dataTotalCountOrders = response1.data.getTotalCountOrder;
  ...
  redirect({
    ...
    history.push(`/orders?pageNum=${pageNum}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`);
  }
                  ...
                  <div className="dataTables_paginate paging_simple_numbers">
                    <ul className="pagination">
                      <li className={`paginate_button page-item ${pageNum - 1 >= 0 ? '' : 'disabled'}`}>
                        {pageNum - 1 >= 0
                          ? (<Link className="page-link" to={`/orders?pageNum=${pageNum - 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>Previous</Link>)
                          : (<div className="page-link">Previous</div>)}
                      </li>
                      <li className="paginate_button page-item">
                        {pageNum - 2 >= 0
                          ? (<Link className="page-link" to={`/orders?pageNum=${pageNum - 2}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum - 1}</Link>)
                          : ''}
                      </li>
                      <li className="paginate_button page-item">
                        {pageNum - 1 >= 0
                          ? (<Link className="page-link" to={`/orders?pageNum=${pageNum - 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum}</Link>)
                          : ''}
                      </li>
                      ...
                      <li className="paginate_button page-item">
                        {(pageNum + 1) * maxPerPage < totalCountOrder
                          ? (<Link className="page-link" to={`/orders?pageNum=${pageNum + 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum + 2}</Link>)
                          : ''}
                      </li>
                      <li className="paginate_button page-item">
                        {(pageNum + 2) * maxPerPage < totalCountOrder
                          ? (<Link className="page-link" to={`/orders?pageNum=${pageNum + 2}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum + 3}</Link>)
                          : ''}
                      </li>
                      <li className={`paginate_button page-item ${(pageNum + 1) * maxPerPage < totalCountOrder ? '' : 'disabled'}`}>
                        {(pageNum + 1) * maxPerPage < totalCountOrder
                          ? (<Link className="page-link" to={`/orders?pageNum=${pageNum + 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>Next</Link>)
