...
const GETTOTALCOUNTORDERSINFERIOR = gql`
query GetTotalCountOrdersfromUserasInferior($userId: Int!){
    getTotalCountOrdersfromUserasInferior(userId: $userId)
  }
`;

const GETORDERSINFERIOR = gql`
  query GetOrdersfromUserasInferior($userId: Int, $pageNum: Int, $maxPerPage: Int, $search: String, $sort: String, $asc: Boolean){
    getOrdersfromUserasInferior(userId: $userId, pageNum: $pageNum, maxPerPage: $maxPerPage, search: $search, sort: $sort, asc: $asc) {
        orderId
        inferior{
          userId
          name
...
class InferiorOrderTable extends React.Component {
  constructor(props) {
    super(props);
  ...
  getOrders({
    ...
    client.query({
      ...
      .then((response) => {
        dataOrders = response.data.getOrdersfromUserasInferior;
        if (!_.isEqual(orders, dataOrders)) {
          client.query({
            query: GETTOTALCOUNTORDERSINFERIOR,
            variables: { userId: self.userId },
            errorPolicy: 'ignore',
          }).then((response1 = response) => {
            dataTotalCountOrders = response1.data.getTotalCountOrdersfromUserasInferior;
  ...
  redirect({
    ...
    history.push(`/orders/inferior?pageNum=${pageNum}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`);
  }
                ...
                <thead>
                  <tr>
                    ...
                    <th
                      style={{ cursor: 'pointer' }}
                      onClick={() => {
                        asccing = sortting === 'SuperiorName' ? !asccing : true;
                        sortting = 'SuperiorName';
                        this.redirect({
                          pageNum, maxPerPage, search, sort: sortting, asc: asccing,
                        });
                      }}
                ...
                <tbody>
                  {orders && orders.map((order, index) => (
                    ...
                    >
                      ...
                      <td>
                        <Link to={`/users/detail?UserId=${order.superior.userId}`}>
                          {order.superior.name}
                        </Link>
                      </td>
                  ...
                  <div className="dataTables_paginate paging_simple_numbers">
                    <ul className="pagination">
                      <li className={`paginate_button page-item ${pageNum - 1 >= 0 ? '' : 'disabled'}`}>
                        {pageNum - 1 >= 0
                          ? (<Link className="page-link" to={`/orders/inferior?pageNum=${pageNum - 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>Previous</Link>)
                          : (<div className="page-link">Previous</div>)}
                      </li>
                      <li className="paginate_button page-item">
                        {pageNum - 2 >= 0
                          ? (<Link className="page-link" to={`/orders/inferior?pageNum=${pageNum - 2}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum - 1}</Link>)
                          : ''}
                      </li>
                      <li className="paginate_button page-item">
                        {pageNum - 1 >= 0
                          ? (<Link className="page-link" to={`/orders/inferior?pageNum=${pageNum - 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum}</Link>)
                          : ''}
                      </li>
                      ...
                      <li className="paginate_button page-item">
                        {(pageNum + 1) * maxPerPage < totalCountOrder
                          ? (<Link className="page-link" to={`/orders/inferior?pageNum=${pageNum + 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum + 2}</Link>)
                          : ''}
                      </li>
                      <li className="paginate_button page-item">
                        {(pageNum + 2) * maxPerPage < totalCountOrder
                          ? (<Link className="page-link" to={`/orders/inferior?pageNum=${pageNum + 2}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>{pageNum + 3}</Link>)
                          : ''}
                      </li>
                      <li className={`paginate_button page-item ${(pageNum + 1) * maxPerPage < totalCountOrder ? '' : 'disabled'}`}>
                        {(pageNum + 1) * maxPerPage < totalCountOrder
                          ? (<Link className="page-link" to={`/orders/inferior?pageNum=${pageNum + 1}&maxPerPage=${maxPerPage}&search=${search}&sort=${sort}&asc=${asc}`}>Next</Link>)
...
export default InferiorOrderTable;
